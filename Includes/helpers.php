...
function handle_create_poll() {
    ...
    if (pvs_verify_request($_POST)) {
        ...
        if (isset($_POST['id'])) {
            ...
            $res = $wpdb->update($wpdb->pvs_question, array('pvs_qid' => $_POST['id'], 'pvs_question' => __($question, 'poll-and-vote-system')), array('pvs_qid' => $_POST['id']));
...
function get_shorcode_content( $attrs ) {
    ...
    ob_start();
    ?>
    <div class="poll_system_block <?php echo $custom_class ?> " id="pvs_block_<?php echo $poll['id']; ?>">
            ...
            foreach ($poll_answers as $answer) {
                ?>
                    <input
                        type='radio'
                        name='pvs_answers'
                        value="<?php echo $answer->pvs_answers; ?>"
                        ...
                        id="pvs_answers_<?php echo $answer->pvs_aid; ?>"/>
                    <label for="answer.pvs_answers_<?php echo $answer->pvs_aid; ?>">
                        <?php echo __($answer->pvs_answers, 'poll-and-vote-system') ?>;
                    </label>
                    <?php
                }
            }   
    ...
    if( $custom_css ) {
        ...
        echo "<style>#pvs_block_$poll_id.poll_system_block .poll_question{font-size:20px;} </style>";
    }
    $poll = [];
    return ob_get_clean();
}
